...
class DashboardController {
  ...
  async getSalesAnalytics(req, res) {
    try {
      ...
      const rows = await sequelize.query(
        `SELECT DATE(p.tanggal_pesan) as date,
                COALESCE(SUM(k.jumlah * m.harga),0) as revenue
         FROM pesanan p
         JOIN keranjang k ON p.id_pesanan = k.id_pesanan
         JOIN menu m ON k.id_menu = m.id_menu
   ...
   async getTopMenus(req, res) {
    try {
      const days = parseInt(req.query.days) || 30;
      const start = moment().subtract(days, "days").format("YYYY-MM-DD");

      const rows = await sequelize.query(
        `SELECT m.nama AS nama_menu,
                COALESCE(SUM(k.jumlah), 0) AS total_terjual
         FROM pesanan p
         JOIN keranjang k ON p.id_pesanan = k.id_pesanan
         JOIN menu m ON k.id_menu = m.id_menu
         WHERE p.tanggal_pesan BETWEEN :start AND DATE_ADD(:end, INTERVAL 1 DAY)
    ...
    try {
      ...
      if (!nama || !harga) {
        return res.status(400).json({
          success: false,
          message: "nama & harga wajib diisi",
        });
      }
